...
S_wind, S_door, S1, S2, S3, S4, S5, S6 = H*L_wind, H*L_door, H*L1, H*L2, H*L3, H*L4, H*L5, H*L6  #surface of walls, windows and doors; in m^2
...
G[1]=(lambda_wall/(2*w_wall)+lambda_insul/(w_insul*2))*S2
G[2]=(lambda_insul/(w_insul*2)+hi)*S2
#wall1
G[3]=(hi+lambda_wall/(2*w_wall))*S1
G[4]=(lambda_wall/(2*w_wall)+lambda_insul/(w_insul*2))*S1
G[5]=(lambda_insul/(w_insul*2)+hi)*S1
#wall5
G[7]=(hi+lambda_wall/(2*w_wall))*S5
G[8]=(lambda_wall/(2*w_wall)+lambda_insul/(w_insul*2))*S5
G[9]=(lambda_insul/(w_insul*2)+hi)*S5
#wall3
G[16]=ho*S3
G[15]=(lambda_wall/(w_wall*2))*S3
G[14]=(lambda_wall/(2*w_wall)+lambda_insul/(w_insul*2))*S3
G[13]=(lambda_insul/(w_insul*2))*S3
G[12]=hi*S3
#wall6
G[23]=ho*S6
G[22]=(lambda_wall/(w_wall*2))*S6
G[21]=(lambda_wall/(2*w_wall)+lambda_insul/(w_insul*2))*S6
G[20]=(lambda_insul/(w_insul*2))*S6
G[19]=hi*S6
#wall4
...
#window
G[17]=(hi+lambda_wind/(w_wind*2))*S_wind
G[18]=(ho+lambda_wind/(w_wind*2))*S_wind
G=np.diag(G)
...
for k in range(n):
    theta[:,k+1]=(eye(dim_thea)+dt*As)*theta[:,k]+dt*Bs*u[:,k]
y=Cs*theta+Ds*us
